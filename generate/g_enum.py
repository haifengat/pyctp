...
def run():
    ...
    f_py_enum = open(os.path.join(cur_dir, '..', 'py_ctp', 'ctp_enum.py'), 'w', encoding='utf-8')  # 增加utf-8解决乱码问题
            ...
            if src_type == 'char' and param == ';':  # char TShfeFtdcInvestorRangeType;
                ...
                for i in range(no, 0, -1):
                    ...
                    if g is not None:  # enum 注释
                        c_lines.append(f'"""{g.group(1)}"""')
                        break
                    # 枚举值
                    g = re.search(r'#define\s*(\S*).*(\w+)', lines[i])  # #define SHFE_FTDC_ISPI_BillDeposit '9'
                    if g is not None:  # 枚举值注释
                        g1 = re.search(r'///(.*)$', lines[i - 1])  # ///结算单结存是否包含质押
                        if g1 is not None:
                            c_lines.append(f'"""{g1.group(1)}"""')
                        c_lines.append(f"{g.group(1)} = {ord(g.group(2))}")
